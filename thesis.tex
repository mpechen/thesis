...
also a vector or just the number in the regression task $y_i \in
\mathcal{R}^n$, or a categorical variable (e.g. ``good/bad'') in the
classification tasks.  The model is trained on the training data set where both
$x_i$ and $y_i$ are known.  On the inference step $x_i$ is known and model is
used to predict $y_i$ for new data samples. An example is a pair $(x_i, y_i)$.
