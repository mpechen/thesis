...
The aim of the machine learning (ML) model in supervised learning task is to predict a target variable $Y \equiv (y_1, \dots, y_n)$ given a set of input features $X \equiv (x_1, \dots, x_n)$.
For example, in the production process $X$ can be sensor readings and $Y$ is a quality measure.
ML model is trained on the training data set where both $X$ and $Y$ are known.
On the inference step $X$ is known and ML model is used to predict $Y$ for new data samples.
...
\begin{figure}[htb!]
	...
	\caption{Illustration from~\cite{gama2014survey} depicting real and virtual concept drift phenomena.}\label{fig:fig1_gama_survey_cd}
\end{figure}
\begin{figure}[htb!]
	\centering
	\includegraphics[width=0.9\textwidth]{images_cropped/gama_survey_cd_fig2}
	\caption{~\cite{gama2014survey}}\label{fig:fig2_gama_survey_cd}
\end{figure}
